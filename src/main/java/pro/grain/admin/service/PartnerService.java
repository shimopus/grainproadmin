...
public class PartnerService {
    ...
    public PartnerDTO save(PartnerDTO partnerDTO) {
        ...
        try {
            ...
            throw new EntityConstrainViolation("Partner", "inn", "unique");
        }
...
//        Page<Partner> result = partnerSearchRepository.search(queryStringQuery(query), pageable);
        ...
        QueryBuilder myQuery = boolQuery().should(queryStringQuery("*" + query + "*").analyzeWildcard(true).field("name").field("inn").boost(5)).
            should(queryStringQuery("*" + query + "*").analyzeWildcard(true).field("card")).
            should(nestedQuery);
